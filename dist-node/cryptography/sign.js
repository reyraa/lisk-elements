...
_tweetnacl2.default.util =
	_tweetnaclUtil2.default; /*
...
var SIGNED_MESSAGE_PREFIX_BYTES = Buffer.from(
	_liskConstants.SIGNED_MESSAGE_PREFIX,
	'utf8',
);
var SIGNED_MESSAGE_PREFIX_LENGTH = (0, _varuintBitcoin.encode)(
	_liskConstants.SIGNED_MESSAGE_PREFIX.length,
);
...
var signMessageWithPassphrase = (exports.signMessageWithPassphrase = function signMessageWithPassphrase(
	message,
	passphrase,
) {
	var msgBytes = digestMessage(message);

	var _getPrivateAndPublicK = (0,
		_keys.getPrivateAndPublicKeyBytesFromPassphrase)(passphrase),
...
var verifyMessageWithPublicKey = (exports.verifyMessageWithPublicKey = function verifyMessageWithPublicKey(
	_ref,
) {
	...
	return _tweetnacl2.default.sign.detached.verify(
		msgBytes,
		signatureBytes,
		publicKeyBytes,
	);
...
var signMessageWithTwoPassphrases = (exports.signMessageWithTwoPassphrases = function signMessageWithTwoPassphrases(
	message,
	passphrase,
	secondPassphrase,
) {
	var msgBytes = digestMessage(message);
	var keypairBytes = (0, _keys.getPrivateAndPublicKeyBytesFromPassphrase)(
		passphrase,
	);
	var secondKeypairBytes = (0, _keys.getPrivateAndPublicKeyBytesFromPassphrase)(
		secondPassphrase,
	);

	var signature = _tweetnacl2.default.sign.detached(
		msgBytes,
		keypairBytes.privateKey,
	);
	var secondSignature = _tweetnacl2.default.sign.detached(
		msgBytes,
...
var verifyMessageWithTwoPublicKeys = (exports.verifyMessageWithTwoPublicKeys = function verifyMessageWithTwoPublicKeys(
	_ref2,
) {
	...
	if (
		secondSignatureBytes.length !== _tweetnacl2.default.sign.signatureLength
	) {
		throw new Error(
...
var printSignedMessage = (exports.printSignedMessage = function printSignedMessage(
	_ref3,
) {
...
var signAndPrintMessage = (exports.signAndPrintMessage = function signAndPrintMessage(
	message,
	passphrase,
	secondPassphrase,
) {
...
var signDataWithPrivateKey = (exports.signDataWithPrivateKey = function signDataWithPrivateKey(
	data,
	privateKey,
) {
...
var signDataWithPassphrase = (exports.signDataWithPassphrase = function signDataWithPassphrase(
	data,
	passphrase,
) {
	var _getPrivateAndPublicK2 = (0,
		_keys.getPrivateAndPublicKeyBytesFromPassphrase)(passphrase),
...
var verifyData = (exports.verifyData = function verifyData(
	data,
	signature,
	publicKey,
) {
	return _tweetnacl2.default.sign.detached.verify(
		data,
		(0, _convert.hexToBuffer)(signature),
		(0, _convert.hexToBuffer)(publicKey),
	);
});
